    ...
    <div class="main-panel">
      ...
      <?php
        ...
        if (isset($_GET['id'])) {
          ...
          $stid = oci_parse($koneksi,"SELECT * FROM PETUGAS WHERE ID_PETUGAS = '$id'");
          oci_execute($stid);
         while (($d = oci_fetch_array($stid, OCI_BOTH)) != false) {
          ?>
      <form class="form-horizontal" action="proses_update_admin.php?act=update" method="post">
       <div class="form-row">
        
        
          <input type="hidden" class="form-control" name="id_petugas" value="<?= $d['ID_PETUGAS'] ?>" >
        <div class="form-group col-md-6">
          <label for="">ID_PETUGAS</label>
          <input type="text" class="form-control" name="id_petugas" value="<?= $d['ID_PETUGAS'] ?>">
        </div>
        <div class="form-group col-md-6">
